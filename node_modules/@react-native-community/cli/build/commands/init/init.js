...
function sanitizeConfigFile(fileContent) {
...
*/
function createDefaultConfigFile(directory, loader) {
  ...
  const configFileContent = `module.exports = ${JSON.stringify(cliConfigContent, null, 2)}`;
  const filepath = 'react-native.config.js';
  try {
    if (!doesDirectoryExist(_path().default.join(directory, filepath))) {
      _fsExtra().default.writeFileSync(filepath, sanitizeConfigFile(configFileContent), {
        encoding: 'utf-8'
      });
    } else {
      const existingConfigFile = require(_path().default.join(directory, filepath));
      const mergedConfig = (0, _deepmerge().default)(existingConfigFile, cliConfigContent);
      const output = `module.exports = ${JSON.stringify(mergedConfig, null, 2)};`;
      _fsExtra().default.writeFileSync(filepath, sanitizeConfigFile(output), {
        encoding: 'utf-8'
      });
    }
    loader.succeed();
  } catch {
    loader.warn(`Could not create custom ${_chalk().default.bold('react-native.config.js')} file. You can create it manually in your project's root folder with the following content: \n\n${configFileContent}`);
  }
}
